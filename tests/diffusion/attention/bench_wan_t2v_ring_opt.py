...
ring-attention implementation:
  ...
  * baseline  (--opt 0): the original ``ring_flash_attn_func_org``
  * optimized (--opt 1): the canonical ``ring_flash_attn_func`` (packed-KV
               double-buffered comm + fused Triton merge + persistent workspace)

Core always calls the canonical (optimized) ``ring_flash_attn_func`` — there is
no runtime toggle. To measure the baseline end-to-end, the ``--opt 0`` leg
injects a ``sitecustomize`` on PYTHONPATH that rebinds ``ring_flash_attn_func``
back to ``ring_flash_attn_func_org`` inside every (spawned) engine worker; the
optimized leg leaves the engine untouched (see ``_install_baseline_ring_injection``).
...
baseline/optimized selection are set cleanly per run. Ring SP is exercised via
``--ring-degree N`` (N GPUs). The A14B backbone (two 14B experts, ~54 GiB in
...
def _install_baseline_ring_injection():
    """Force the engine's spawned workers onto the ORIGINAL ring path (A/B baseline).

    Core hard-wires the optimized ``ring_flash_attn_func`` — there is no runtime
    toggle (per review: the library reads no env vars). The diffusion engine spawns
    its worker ranks with ``multiprocessing`` ``spawn`` (fresh interpreters), so an
    in-process monkeypatch here would never reach them. Instead we drop a
    ``sitecustomize.py`` in a temp dir and prepend it to ``PYTHONPATH``: every
    spawned worker auto-imports it at interpreter startup (``spawn`` inherits the
    env) and, guarded by ``WAN_BENCH_FORCE_BASELINE_RING``, rebinds the module
    attribute ``ring_flash_attn_func`` -> ``ring_flash_attn_func_org`` (core does a
    fresh ``from ... import ring_flash_attn_func`` per call, so it picks up the
    swap). Confined entirely to this benchmark; the library is untouched. Call in
    the ``--opt 0`` leg BEFORE constructing ``Omni`` so the workers inherit the env.
    """
    inject_dir = tempfile.mkdtemp(prefix="wan_ring_baseline_")
    with open(os.path.join(inject_dir, "sitecustomize.py"), "w") as f:
        f.write(
            "# Auto-generated by bench_wan_t2v_ring_opt.py (baseline A/B leg).\n"
            "# Rebinds the canonical ring attn back to the original impl in every\n"
            "# spawned diffusion worker. No-op unless the guard env var is set.\n"
            "import os\n"
            "if os.environ.get('WAN_BENCH_FORCE_BASELINE_RING') == '1':\n"
            "    try:\n"
            "        from vllm_omni.diffusion.attention.backends import ring_flash_attn as _r\n"
            "        _r.ring_flash_attn_func = _r.ring_flash_attn_func_org\n"
            "    except Exception:\n"
            "        pass\n"
        )
    os.environ["WAN_BENCH_FORCE_BASELINE_RING"] = "1"
    prev = os.environ.get("PYTHONPATH", "")
    os.environ["PYTHONPATH"] = os.pathsep.join([inject_dir, prev]) if prev else inject_dir
    return inject_dir
...
# ===========================================================================
# Worker: one generation run under a fixed ring-attention implementation.
# ===========================================================================
def run_worker(args):
    # Core always uses the optimized ``ring_flash_attn_func``; there is no runtime
    # toggle. For the baseline A/B leg, inject a sitecustomize (before Omni spawns
    # its worker ranks, so they inherit PYTHONPATH) that swaps it back to the
    # original ``ring_flash_attn_func_org``.
    if args.opt == 0:
    ...
    # Wan2.2 resolves through the pipeline registry, whose stage-config path only
    # honors the *flat* diffusion-parallel fields (ring_degree/ulysses_degree/
    # use_hsdp/...) declared on StageDeployConfig, and takes the worker GPU set from
    # the stage ``devices`` string. A pre-built DiffusionParallelConfig object is
    # silently dropped there (build_stage_runtime_overrides drops the orchestrator-
    # owned ``parallel_config`` key) and ``devices`` defaults to a single GPU, which
    # collapses the run to ring=1 on GPU0. So pass the flat fields + an explicit
    # ``devices`` list instead of a parallel_config object.
    omni_kwargs = dict(
        model=args.model,
        ...
        vae_use_tiling=True,
        # Loading the A14B backbone (two 14B experts, ~108 GiB of shards on disk)
        # plus the engine's profiling forward blows past the 600s default.
    ...
    print(
        f"[worker opt={args.opt}] frames={frames.shape} "
        f"times={[f'{t:.3f}' for t in times]}s "
...
def run_driver(args):
    ...
    print(f"{'#' * 70}")
    print(
        f"config: ring_degree={args.ring_degree}  hsdp={args.use_hsdp}  steps={args.steps}  "
    ...
    verdict = (
        "PASS (visually identical)"
        if (psnr >= 40.0 or max_diff <= 2)
        else "REVIEW (diff larger than bf16 round-off expectation)"
    )
    print(f"  -> {verdict}")
    print("-" * 70)
    ...
    print(f"  baseline runs : {[f'{t:.3f}' for t in results[0]['times']]}")
    print(f"  optimized runs: {[f'{t:.3f}' for t in results[1]['times']]}")
    print(f"{'#' * 70}")
...
def parse_args():
    ...
    p.add_argument(
        "--prompt", default="Two anthropomorphic cats in boxing gear fight intensely on a spotlighted stage."
    )
